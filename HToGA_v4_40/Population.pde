class Population {
  ...
  void nsga2(){
    ...
    for (int rbtidx = 0; rbtidx < unsortRobotList.size(); rbtidx++){
      ...
      /*
        print("RobotID " + nf(curGARobot.popID,3)  + " reach " + curGARobot.reachedGoal + " score: " + nf(curGARobot.costScore,2,2) + " cmds: " );
        String toWrite;
        toWrite = "";
    ...
    // At least a robot reached goal, compare through multi-objective function  
      ...
      println("=====================");
            ...
            while (parentRobotList.size() < genSize){
              ...
              //println("sutck here, size:" + parentRobotList.size() + " parentRbt.reachedGoal " + parentRbt.reachedGoal);
              //if (!parentRbt.reachedGoal){
              parentRobotList.add(parentRbt);
              //}
              
              //for (int costidx = arCost.size()-1; costidx >= lowerBound; costidx--){
              //  if (!arCost.get(costidx).reachedGoal){
              //    parentRobotList.add(arCost.get(costidx));
              //  }
              //}
            }
  ...
  //mutate
  void GAMutation() {
    for (int i = 1; i < newMutRbts.length; i++) {
