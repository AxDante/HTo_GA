class Robot {
  ...
  PVector mutateMove(PVector vel){
    ...
    for (int grididx = 0; grididx < nextGrid.length; grididx++){
      for (int blkidx = 0; blkidx < 4; blkidx++){
        ...
        nextGridScore[grididx] *= (float)(10/(1+getGridFitnessValue(gridID[0],gridID[1], 1)));
  ...
  PVector mutateMoveDis(PVector vel, int sd){
    ...
    for (int grididx = 0; grididx < nextGrid.length; grididx++){
      ...
      if (vel.copy().dist(fourDirArray[grididx]) == 0 && nextGridScore[grididx] < 1){
        safeScore *= 0.9;
      }
    }
    safeScore *= 0.9;
    ...
    //int selectidx = PortionSelect(nextGridScore);
    //nvel = fourDirArray[selectidx];
    //safeScore += nextGridScore[selectidx];
    //safeScore *= 0.99;
    nvel = vel.copy();
    return nvel;
  }
  ...
  void setScore(int scoreID_, float costScore_, float smoothScore_, float safeScore_){
    scoreID = scoreID_;
