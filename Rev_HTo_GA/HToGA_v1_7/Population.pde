class Population {
  ...
  Population(int size) {
    ...
    StepData = new float[maxStep];
    StepDataCD = new float[maxStep];
    historyBestScore = new float[maxStep];
    for (int i = 0; i < StepData.length; i++) {
      StepData[i] = -1;
      StepDataCD[i] = -1;
      historyBestScore[i] = -1;
    }
  ...
  void show() {
    Rbts[0].show();
    for (int i = 1; i< floor(Rbts.length); i++) {
      Rbts[i].show();
    }
  }

  void updateBlkGridPos(){
    for (int rbtidx = 0; rbtidx < Rbts.length; rbtidx++) {
      for (int blkidx = 0; blkidx < 4; blkidx++){
        Block blk = Rbts[rbtidx].Blks[blkidx];
        blk.isBlkGridPosValid = true;
        int[] tempGridPos = new int[2];
        tempGridPos[0] = floor(blk.pos.x/blkW);
        tempGridPos[1] = floor(blk.pos.y/blkW);
        if (tempGridPos[0] >= mapW || tempGridPos[0] < 0){
          blk.isBlkGridPosValid = false;
        }
        if (tempGridPos[1] >= mapH || tempGridPos[1] < 0){
          blk.isBlkGridPosValid = false;
        }
        if(blk.isBlkGridPosValid){
          arrayCopy(tempGridPos, blk.blkGridPos);
        }
      }
    }
  }
  ...
  void statusCheck(){
    ...
    for (int rbtidx = 0; rbtidx < Rbts.length; rbtidx++) {
      ...
      if (populationTime >= maxTime){
        curRbt.isDead = true;
        curRbt.deadCause = "Overtime";
        continue;
      } 
      if (curRbt.brain.Cmds.size() < populationTime){
        curRbt.isDead = true;
        curRbt.deadCause = "ReachGoal";
        continue;
      } 
      if (isCollideRbt(Rbts[rbtidx])){
        curRbt.isDead = true;
        curRbt.deadCause = "Collision";
        continue;
      } 
      
      curRbt.deadCause = "None";
      ...
      if (dist(curRbtpos.x, curRbtpos.y, nextWppos.x, nextWppos.y) < arrivalDistance) {
        ...
        for(int cmdidx = curRbt.brain.Cmds.size()-1; cmdidx > populationTime; cmdidx--){  // Remove command length TODO: may need adjust
          curRbt.brain.Cmds.remove(cmdidx);
        }
      }else{
        curRbt.isMovePermitted = true;
      }
  ...
  void moveAllRbts(){
    ...
    for (int rbtidx = 0; rbtidx < Rbts.length; rbtidx++) {
      curRbt = Rbts[rbtidx];
      if (curRbt.isMovePermitted){
        curRbt.move();
        curRbt.isMovePermitted = false;
      }
    }
  }
  ...
  void update() {
    println("a");
    moveAllRbts();      // Move all robots
    println("b1");
    updateBlkGridPos();
    println("b2");
    statusCheck();      // Check robot status (position, isDead)
    println("c");
    debugMode();        // Print robot status
    populationTime += 1;
  ...
  void debugMode(){
    Robot curRbt;
    println("=================================");
    println("Population time: " +populationTime + "   | systime: " + time);
    ...
    for (int rbtidx = 0; rbtidx < Rbts.length; rbtidx++) {
      ...
      println("isDead: " + curRbt.isDead + "\t | Cause: " + curRbt.deadCause);
  ...
  void calcFitness() {
    ...
    for (int i = 0; i< Rbts.length; i++) {
      ...
      if (Rbts[i].isReachGoal){
        tempFitness = 1+100.0/(float)(rbtCmdSize);
        Rbts[i].isFinalFitness = true;
      }else if (Rbts[i].isDead){
        Rbts[i].isFinalFitness = true;
        continue;
      }else {
  ...
  boolean allRobotsDead() {
    for (int i = 0; i< Rbts.length; i++) {
      if (!Rbts[i].isDead && !Rbts[i].isReachGoal) { 
        return false;
      }
    }
    return true;
  ...
  boolean isConverged() {
    ...
    if (gen-1 > minConvergeSample) {
      ...
      if (StepData[gen-1-minConvergeSample] != -1 &&
          StepData[gen-1] == StepData[gen-3] && StepData[gen-1] == StepData[gen-5] &&
          //StepDataCD[gen-1] == StepDataCD[gen-3] &&
          historyBestScore[gen-1] == historyBestScore[gen-5]){
        return true;
      }
    }
    return false;
  ...
  void nsga2(){
    ...
    if (nonReached){
      ...
      int costidx = arCost.size()-1;
      while (parentRobotList.size() < genSize){
        boolean noRepeat = true;
        for (int listidx = 0; listidx < parentRobotList.size(); listidx++){
          if (arCost.get(costidx).costScore == parentRobotList.get(listidx).costScore &&
              arCost.get(costidx).bestFitnessTime == parentRobotList.get(listidx).bestFitnessTime){
            noRepeat = false;
           }
        }
        if (noRepeat){
    ...
    // At least a robot reached goal, compare through multi-objective function  
      ...
      while(countLength < requiredLength){
        ...
        for (int rbtidx = 0; rbtidx < unsortRobotList.size(); rbtidx++){
          ...
          if (paretoBest && curGARobot.isReachGoal){
            boolean isRepeat = false;
            if (curGARobot.sumScore > historyBestScore[gen]) historyBestScore[gen] = curGARobot.sumScore;
            if (frontCount == 0){
      ...
      if(dispPopResult){
        ...
        println("bestSumCDRbtScore: " + unsortRobotListCopy.get(bestSumCDRbtID).sumScore + " ; bestSumRbtScore:" + unsortRobotListCopy.get(bestSumRbtID).sumScore + " ; histbestSumRbtScore:" + historyBestScore[gen]);        
        println("=====================");
      }
  ...
  boolean naturalSelection() {
    ...
    if (!nonReached){
      if (unsortRobotListCopy.get(bestSumCDRbtID).isReachGoal) {
        StepData[gen] = unsortRobotListCopy.get(bestSumRbtID).sumScore; 
        StepDataCD[gen] = unsortRobotListCopy.get(bestSumCDRbtID).sumScore;    
      }
    }
    ...
    //newRbts[0] = unsortRobotList.get(nrBestCostRbtID).generateChild();
    for  (int i = 0; i < newRbts.length; i++) {
      Robot parent = selectParent(parentRobotList);
      newRbts[i] = (Robot) parent.generateChild();
      newMutRbts[i] = (Robot) parent.generateChild();
      newSwapRbts[i] = (Robot) parent.generateChild();
      newPercRbts[i] = (Robot) parent.generateChild();
    }  
  ...
  void MergeRobots(){
    ...
    for (int i = 0; i < genSize; i++) {
      Rbts[i] = (Robot) parentRobotList.get(i).generateChild();
      Rbts[i].generateType = "Parent copy";
    }
    for (int i = genSize; i < genSize*2; i++) {
      Rbts[i] = (Robot) newRbts[i-genSize].generateChild();
      Rbts[i].generateType = "New robot";
    }  
    for (int i = genSize*2; i < genSize*3; i++) {
      Rbts[i] = (Robot) newMutRbts[i-genSize*2].generateChild();
      Rbts[i].generateType = "New mutated robot";
    }  
    for (int i = genSize*3; i < genSize*4; i++) {
      Rbts[i] = (Robot) newSwapRbts[i-genSize*3].generateChild();
      Rbts[i].generateType = "New swaped robot";
    }  
    for (int i = genSize*4; i < genSize*5; i++) {
      Rbts[i] = (Robot) newPercRbts[i-genSize*4].generateChild();
      Rbts[i].isPerc = true;
      Rbts[i].generateType = "New perception robot";
    }  
  }
  ...
  void GANonreached(){
    ...
    for (int i = 0; i < newSwapRbts.length; i++) {
      for (int cmdidx = 0; cmdidx < newSwapRbts[i].brain.Cmds.size(); cmdidx++) {
        ...
        if (rand < baseMutTransRate && mutTransProcess && cmdidx!= 0 && cmdidx != newSwapRbts[i].brain.Cmds.size()-1){
          ...
          if (randMorph == 3 || randMorph == 4 || randMorph == 5 ) randMorph = 1;
          if (randMorph == 6 || randMorph == 7 ) randMorph = 8;
    ...
    for (int i = 0; i < newRbts.length; i++) {
      Robot parent = selectParent(parentRobotList);
      newRbts[i] = parent.generateChild();
    }
  }
  ...
  void GASwap() {
    ...
    */
    for (int i = 0; i< newSwapRbts.length; i++) {
        ...
        while (!noAdjust && count < 120){
          count++;
          noAdjust = true;
          int cmdSize = newSwapRbts[i].brain.Cmds.size();
          int randDist = floor(random(8))+1;  //1 or 2 or ... or 8
          for (int cmdidx = cmdSize-randDist; cmdidx >= randDist; cmdidx-=1){
            String str1 = newSwapRbts[i].brain.Cmds.get(cmdidx);
            String str2 = newSwapRbts[i].brain.Cmds.get(cmdidx-randDist);
            if ((str1 == "F" && str2 == "B")||(str1 == "B" && str2 == "F")||(str1 == "R" && str2 == "L")||(str1 == "L" && str2 == "R")){
              newSwapRbts[i].brain.Cmds.remove(cmdidx);
              newSwapRbts[i].brain.Cmds.remove(cmdidx-randDist);
              noAdjust = false;
              break;
            }
          }
          ...
          for (int cmdidx = cmdSize-1; cmdidx >= 1; cmdidx-=1){
            ...
            if ((str1 == "F" && str2 == "B")||(str1 == "B" && str2 == "F")||(str1 == "R" && str2 == "L")||(str1 == "L" && str2 == "R")){
              ...
              if (nonReached){
                ...
                if (randFloat < randAddBack){
                  int randInt = floor(random(4));
                  newRbts[i].brain.Cmds.add(fourDirString[randInt]);
                  newRbts[i].brain.Cmds.add(fourDirString[randInt]);
                }
              }
              break;
  ...
  void nsga2RecordData(){
    ...
    float revGridScore = pow(1/unsortRobotListCopy.get(nrBestCostRbtID).costScore - 1, 0.5);
    float revGRFitness = 1-revGridScore/maxTime;
    float revSFitness = 1-revGridScore/maxTime;
    
    if (!nonReached){
      if (!isReachRecord){
        reachTime = millis();
        trialReachTime[intTrials] = reachTime - startTime;
        trialReachGen[intTrials] = gen;
        isReachRecord = true;
      }
      ...
      newRow.setFloat("f_t", 1-(float)unsortRobotListCopy.get(bestSumCDRbtID).brain.Cmds.size()/maxTime);
      newRow.setFloat("f_sm", unsortRobotListCopy.get(bestSumCDRbtID).smoothScore*2-1);
      ...
      newRow.setFloat("f_gr", 1/(1+pow(1/unsortRobotListCopy.get(nrBestCostRbtID).costScore-1, 0.5)));
      newRow.setFloat("f_t", 1-(float)unsortRobotListCopy.get(nrBestCostRbtID).brain.Cmds.size()/maxTime);
      newRow.setFloat("f_sm", unsortRobotListCopy.get(nrBestCostRbtID).smoothScore*2-1);
