...
int totTrials = 5;                    // Total number of trials (default: 50)
...
void setup(){
  ...
  trialTime = new double[totTrials];
  trialReachTime = new double[totTrials];
  trialReachPop = new int[totTrials];
  trialTermPop = new int[totTrials];
...
void mainLoop(){
  ...
  if (intTrials < totTrials){
    ...
    if (trialComplete){
      ...
      terminateTrial = true;
      finalTime = millis();
      trialTime[intTrials] = finalTime - startTime;
      trialTermPop[intTrials] = test.gen;
      ...
      if (intTrials < totTrials){
        ...
        for(int inttr = 0; inttr < totTrials; inttr++){
          ...
          println("reach time: "+ trialReachTime[inttr] + " term time: "+ trialTime[inttr]);
          println("reach gen: " + trialReachPop[inttr] + " term gen: " + trialTermPop[inttr]);
        }
      }
    }
  }
