class Population {
  ...
  Population(int size, Obstacle[] Obss_) {
    ...
    //bestTime = maxStep_;
    genSize = size;
    Rbts = new Robot[genSize*6];
    for (int i = 0; i < genSize*6; i++) {
      Rbts[i] = new Robot();
    }
  ...
  boolean isConverged() {
    ...
    if (gen-1 > minConvergeSample) {
      ...
      if (StepData[gen-1-minConvergeSample] != -1 &&
          StepData[gen-1] == StepData[gen-3] && StepData[gen-1] == StepData[gen-5] && StepData[gen-1] == StepData[gen-7] &&
  ...
  boolean naturalSelection() {
    ...
    newTwoDirRbts = new Robot[genSize];
    ...
    for  (int i = 0; i < newRbts.length; i++) {
      ...
      newTwoDirRbts[i] = (Robot) parent.gimmeBaby();
    }  
  ...
  void MergeRobots(){
    ...
    for (int i = genSize*5; i < genSize*6; i++) {
      Rbts[i] = (Robot) newTwoDirRbts[i-genSize*5].gimmeBaby();
    }  
  }
  ...
  void GANonreached(){
    ...
    for (int i = 0; i < newSwapRbts.length; i++) {
      for (int cmdidx = 0; cmdidx < newSwapRbts[i].brain.Cmds.size(); cmdidx++) {
        float rand = random(1);
        float mutTransRate = baseMutTransRate*(1+cmdidx/(float)(newMutRbts[i].bestFitnessTime+1));
        if (rand < mutTransRate && mutTransProcess && cmdidx!= 0 && cmdidx != newSwapRbts[i].brain.Cmds.size()-1){
  ...
  void GAMutation() {
    for (int i = 0; i < newMutRbts.length; i++) {
      for (int cmdidx = 0; cmdidx < newMutRbts[i].brain.Cmds.size(); cmdidx++) {
        float rand = random(1);
        float mutTransRate = baseMutTransRate*(1+cmdidx/(newMutRbts[i].bestFitnessTime+1));
        if (rand < mutTransRate && mutTransProcess && cmdidx!= 0 && cmdidx != newMutRbts[i].brain.Cmds.size()-1){
          int randMorph = floor(random(morphNum));
  ...
  void GAReduce() {
    for (int i = 0; i< newTwoDirRbts.length; i++) {
      int cmdSize = newTwoDirRbts[i].brain.Cmds.size();
      int count = 0;
      while (count < 50){
        count++;
        int idx1 = floor(random(cmdSize-8)+4);
        int idx2 = floor(random(8)-4);
        String str1 = newTwoDirRbts[i].brain.Cmds.get(idx1);
        String str2 = newTwoDirRbts[i].brain.Cmds.get(idx1+idx2);
        if (idx1 != idx2){
          if ((str1 == "F" && str2 == "B")||(str1 == "B" && str2 == "F")||(str1 == "R" && str2 == "L")||(str1 == "L" && str2 == "R")){
            if(idx2 < 0){
              newTwoDirRbts[i].brain.Cmds.remove(idx1);
              newTwoDirRbts[i].brain.Cmds.remove(idx1+idx2);
            }else{
              newTwoDirRbts[i].brain.Cmds.remove(idx1+idx2);
              newTwoDirRbts[i].brain.Cmds.remove(idx1);
            }
            if (nonReached){
              int randInt = floor(random(4));
              newTwoDirRbts[i].brain.Cmds.add(fourDirString[randInt]);
              newTwoDirRbts[i].brain.Cmds.add(fourDirString[randInt]);
            }
            break;
          }
  ...
  void GARemoveExtraShapes() {
    ...
    for (int i = 0; i< newRbts.length; i++) {
      ...
      for (int cmdidx = cmdSize-1; cmdidx >= 0; cmdidx--){
         int inMorph = int(newRbts[i].brain.Cmds.get(cmdidx));
         if (inMorph != 0 || newRbts[i].brain.Cmds.get(cmdidx).equals("0")){
