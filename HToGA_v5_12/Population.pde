  ...
  void calculateFitness() {
    for (int i = 0; i< Rbts.length; i++) {
      ...
      if (Rbts[i].reachedGoal) {
        ...
        tempSmoothScore = (rbtCmdSize/(float)(rbtCmdSize+disSim)); ///rbtCmdSize/rbtCmdSize; //(float)Rbts[i].brain.Cmds.size();
        ...
        tempSmoothScore = (rbtCmdSize/(float)(rbtCmdSize+disSim)); //(float)Rbts[i].brain.Cmds.size();
