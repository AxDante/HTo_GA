class Robot {
  ...
  void move() {
    updateBlockDesHeading();
    if (brain.Cmds.length > time) {  //if there are still directions left then set the acceleration as the next PVector in the direcitons array
      if (!shapeShift()){
        if (!gridBasedMode){
  ...
  void update() {
    if (!dead && !reachedGoal) {
      ...
      else if (dist(Blks[1].pos.x, Blks[1].pos.y, map.Wps[currentWpID+1].pos.x, map.Wps[currentWpID+1].pos.y) < 5) {//if reached goal
        reachedGoal = true;
      }
    }
  }
