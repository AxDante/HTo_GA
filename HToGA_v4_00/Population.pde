class Population {
  ...
  Population(int size, int minStep_,  Obstacle[] Obss_) {
    ...
    Front = new ArrayList<int[]>();
    Front.add(new int[]{0, 13, 24, 55});
    Front.add(new int[]{12, 164, 75, 15, 234});
  ...
  int nsga2(){
    ...
    for (int frontidx = 0; frontidx < maxFront; frontidx++){
      ...
      // For each robot in the generation
      int unsortRobotListSize = unsortRobotList.size();
      int rbtidx = 0;
      while (rbtidx < unsortRobotListSize){
        
        for (int cmpidx = 0; cmpidx < Front.get(frontidx).length; cmpidx++){
          boolean paretoImprove = true;
          GARobot curGARobot = unsortRobotList.get(rbtidx);
          GARobot cmpGARobot = findGARobot(Front.get(frontidx)[cmpidx]);
