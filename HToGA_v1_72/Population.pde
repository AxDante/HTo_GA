  ...
  void naturalSelection() {
    ...
    if (Rbts[bestRobot].reachedGoal) {
      ...
      for (int cmdidx = 0; cmdidx < cmds.length; cmdidx++){
        String[] dirArray = new String[]{"B", "F", "L", "R", "-"};
        PVector[] randDirArray = new PVector[]{new PVector(0,blkWidth), new PVector(0,-blkWidth), new PVector(blkWidth,0), new PVector(-blkWidth,0), new PVector(0,0)};
        for (int arridx = 0; arridx < dirArray.length; arridx++){
          if (randDirArray[arridx].dist(cmds[cmdidx].moveDir) == 0){
            print(dirArray[arridx] + " ");
          }
        }
        //print("(" + cmds[cmdidx].moveDir.x +"," + cmds[cmdidx].moveDir.y + ")  " + cmds[cmdidx].transMorph + "   ");
        if ((cmdidx+1)%50 == 0){
          print("\n");
        }
      }
